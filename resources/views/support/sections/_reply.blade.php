<div class="mb-6 flex items-start">
    <div class="flex-no-shrink h-8 w-8">
        <a class="bg-blue-200 shadow-none px-0 h-8 w-8 flex items-center justify-center rounded-full overflow-hidden dropdown-toggle" href="#">
            <img class="h-8 w-8" src="{{ ($reply->agent ?: $reply->customer)->image ?? asset('img/default.jpg') }}">
        </a>
    </div>
    ...
    <div class="ml-6 text-gray-700">
        <div class="mt-1 leading-none">
            <h6 class="font-semibold text-gray-800 flex items-center">
                {{ ($reply->agent ?: $reply->customer)->name }}

                @if ($reply->agent)
                    <span class="ml-2 px-2 inline-flex text-xs leading-5 font-medium rounded-full bg-yellow-100 text-yellow-800">Support agent</span>
                @endif
            </h6>
        </div>
...
<div class="modal fade" id="replyModal" tabindex="-1" aria-labelledby="replyModalLabel" aria-hidden="true">
    <div class="modal-dialog rounded-lg overflow-hidden">
        <form class="modal-content" action="{{ route('replies.update', ['ticket' => $ticket, 'reply' => $reply]) }}" method="POST">
